...
- **Right tool per step:** `query_san`/`semantic_search_nodes` to **FIND** which file/symbol you need → `get_san(file_path=...)` to **READ/understand** it (not `cat`/`head`/`sed` on the file) → `grep`/`rg` on a file only to locate an **exact literal** SAN abstracts away → raw `Read` only to **EDIT** (need exact bytes). The shell stays fine for discovery grep across files and for build/run/git/test; just don't `cat`/`sed` a source file that has a fresh `.san` to understand it.
