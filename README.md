...
2. **Install the managed compiler adapter** for your host (once):
   ```bash
   ./setup.sh --claude    # or --codex, or --all
   ```
   This installs a managed `brain-compiler` agent/skill rendered from the
   provider-neutral contract `san/compiler-contract.md`, pinned to the
   configured model (see [`docs/adapters.md`](docs/adapters.md)).

3. **Generate SAN files** by invoking that adapter on your host:
   ```
   # e.g. "Convert src/services/AuthService.kt to SAN"
   ```
   The adapter reads the source, then calls the server's `publish_san` tool,
   which validates the content and atomically writes
   `your-repo/.san/src/services/AuthService.kt.san` (the source path with
   `.san` appended — mirrors the source tree). The server itself never invokes
   a model.
...
5. **Query SAN:**
   ...
   plan_san_refresh("my-backend")               # read-only: what needs (re)generation
   recompile_san("my-backend", dry_run=True)    # human-readable freshness report
   ```
...
|---------|-------------|
| `plan_san_refresh("repo")` | **Read-only** structured plan: source digests + which SANs are missing/stale/fresh/orphaned/unsupported/malformed. Zero filesystem or metrics mutation. |
| `publish_san(repo, source_path, expected_source_sha256, san_content, provider, model, reasoning_effort=None)` | The host adapter's write path: validate + atomically publish one SAN, bound to the source digest. Rejects config drift, traversal, stale digests, invalid content; rolls back on any failure. |
| `recompile_san("repo", dry_run=True)` | Human-readable freshness report (read-only; delegates to `plan_san_refresh`) |
...
SAN content is generated by the **host** (an LLM — Claude Code or Codex), never by the agent-brain server. The server plans, validates, and publishes; it never invokes a model or a provider CLI. The division:

- **Host** (via its managed `brain-compiler` adapter): reads source, emits SAN, calls `publish_san`.
- **Server**: `plan_san_refresh` (what needs work), `publish_san` (validate + atomically write, digest-bound), staleness/orphan housekeeping.

**Workflow:**
1. `plan_san_refresh("repo")` reports missing/stale files with their source SHA-256.
2. The active host's `brain-compiler` adapter generates SAN for each and calls `publish_san(...)` with the source digest.
3. `publish_san` validates the candidate structurally, verifies the source hasn't changed (digest compare-and-swap), then atomically replaces the SAN and updates the hash + index. On any failure it rolls back all prior state.
4. `query_san` / `get_san` read the published SAN; `plan_san_refresh` re-reports what's still stale.

See **[`docs/adapters.md`](docs/adapters.md)** for the full provider-aware flow, managed paths, config, and the no-fallback policy.

### Which model runs the compiler

The compiler model is **configured, not chosen at call time**, and there is **no automatic fallback** — if the configured model is unavailable the adapter stops rather than silently downgrading. Defaults (override in `config.json` under `san_compiler`):

| Provider | Default model | Reasoning effort |
|----------|---------------|------------------|
| Claude | `claude-sonnet-4-6` | — |
| Codex | `gpt-5.4-mini` | `medium` |

SAN conversion is mechanical (read source → emit facts), so a cheap tier is the right default; your engineering agents *consuming* SAN can run on bigger models since SAN cuts their input cost to ~20% of raw anyway. To change the compiler model, edit `config.json` and re-run `./setup.sh` for that provider (managed adapters re-render; an unmarked hand-edited adapter is preserved and reported, never overwritten).

### Other hosts

The **MCP server is host-agnostic** — any MCP client can call `plan_san_refresh` / `publish_san` / `query_san` / `get_san` / `recompile_san`. The SAN grammar itself is defined once in the provider-neutral contract [`san/compiler-contract.md`](san/compiler-contract.md); each host gets a managed adapter rendered from it. To wire a host that `setup.sh` doesn't yet manage, point it at that contract and have it call `publish_san` (mirror the source tree, append `.san`). The server's digest-based staleness tracking works identically regardless of which host wrote the file.
...
> **Important: SAN refresh is NOT automatic.** Staleness checks run when you call `plan_san_refresh`, `query_san`, `get_san`, or `recompile_san(dry_run=True)` — they report stale SANs but do NOT regenerate them. To force a full metadata refresh (e.g., after a large merge or branch switch), call `recompile_san("repo")`. To regenerate stale SAN *content*, invoke your host's `brain-compiler` adapter on the reported files (it calls `publish_san`).

> **`.san/` is local generated output.** SAN files are regenerated from source by the host adapter and published transactionally, so `./setup.sh --link-project` adds `.san/` to your project's `.gitignore` (an existing anchored `/.san/` is accepted). Keep them out of version control; regenerate them per checkout.
